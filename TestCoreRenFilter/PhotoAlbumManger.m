...
@implementation PhotoAlbumManger

+ (instancetype)sharedManager
{
    static PhotoAlbumManger *manager = nil;
...
{
    NSMutableArray<PhotoAlbum *> *mArr = [NSMutableArray array];
    ...
    [smartAlbum enumerateObjectsUsingBlock:^(PHAssetCollection *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
    {
        if (obj.assetCollectionSubtype != 202 && obj.assetCollectionSubtype < 212) {
            ...
            if ([assets count])
            {
                PhotoAlbum *pa = [[PhotoAlbum alloc] init];
